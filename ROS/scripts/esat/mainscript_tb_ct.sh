...
log_ros="$HOME/log/circle_target/tb/logros_$(date +%F_%H%M)"
xterm -l -lf $log_ros -e roslaunch rl_gcg circle_target_tb.launch &
pid_ros=$!
...
log_gcg="$HOME/log/circle_target/tb/loggcg_$(date +%F_%H%M)"
