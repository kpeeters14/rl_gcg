...
def fsm_datapath():
  ...
  # RESET ORIENTATION
  # Reset the orientation of the drone so that it is parallel to the x-axis (positive or negative x depending on yaw)
  elif (state == 4):
    twist.linear.x = 0.0
    twist.linear.y = 0.0
    twist.linear.z = 0.0
    twist.angular.x = 0.0
    twist.angular.y = 0.0
    difference = 1.57 - abs(yaw)
    if (abs(difference) < 1.52):
      if (difference >= 0):
        twist.angular.z = -np.sign(yaw)*0.25
      else:
        twist.angular.z = np.sign(yaw)*0.25
    else:
      twist.angular.z = 0
  ...
  # Reset the position of the drone so that it is more or less in the origin again
    ...
    if (abs(pos_x) > 0.1):
      twist.linear.x = -np.sign(pos_x)*np.sign(difference)*0.1
    else:
      twist.linear.x = 0.0
    if (abs(pos_y) > 0.1):
      twist.linear.y = -np.sign(pos_y)*np.sign(difference)*0.1
    else:
      twist.linear.y = 0.0
    twist.linear.z = 0.0
    twist.angular.x = 0.0
  ...
  # Reset the orientation of the drone so that the target object is in sight again (give it a speed around the z-axis)
    ...
    if (abs(target_yaw - yaw) > 0.05):
      if (difference >= 0):
        twist.angular.z = np.sign(target_yaw)*0.25
      else:
        twist.angular.z = -np.sign(target_yaw)*0.25
    else:
      twist.angular.z = 0
